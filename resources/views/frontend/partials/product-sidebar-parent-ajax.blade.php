...
<div style="display: flex;" class="text-center parent-item">
  @foreach (App\Models\Category::orderBy('name', 'asc')->where('parent_id', NULL)->get() as $parent)
    <a id="parent_{{ $parent->id }}" data-parent-id="{{ $parent->id }}" data-parent-slug="{{ $parent->slug }}" href="{{ route('categories.show.parent',$parent->slug) }}" class="list-group-item list-group-item-action p-0  {{ $parent->id==$id?'active':'' }}">
