<!-- COMMENT -->
      <div style="font-size: 14px">
            ...
            @if($i>0)
              @if(!is_null($rating->comment))
                <li class="par ml-2 mt-3">
                  ...
                  <div class="rc ml-2">
                    ...
                    <div style="margin-top: -12px; color: #999;">
                      <span><a href="javascript:void(0)" class="cmthref" rate_id="{{ $rating->id }}" id="cmt-{{ $rating->id }}" style="color: #007bff">{{ count($comments)>0 ? count($comments):'' }} {{ count($comments)>0 ? 'thảo luận':'Thảo luận' }}</a></span>
                      <span> • </span>
                      <span style="">{{ $rating->updated_at->diffForHumans(Carbon\Carbon::now()) }}</span>
                    </div>
                    @if(count($comments)==0)
                    <div class="mt-2 mb-3 text" id="text-{{ $rating->id }}">
                ...
                @if(count($comments)>0)
                  ...
                  @endforeach                 
                  <div id="cmt-child-rating-{{ $rating->id }}">
                      
                  </div>

                  <li class="par ml-4 mt-3 child-cmt">
                    <div class="mt-3 mb-3 text" id="text-{{ $rating->id }}">
                      <form id="submit_cmtrate-{{ $rating->id }}" action="{{ route('rating.comment') }}" method="post" accept-charset="utf-8" class="form-inline">
                        @csrf
                        <input type="text" class="form-control mr-2" name="cmtrating" value="" placeholder="Nhập thảo luận của bạn" style="font-size: 14px;padding:7px;width: 380px">
                        <input type="hidden" name="rating_id" value="{{ $rating->id }}"> 
        ...
        <a href="{!! route('products.show',$product->slug) !!}#view-rating" class="rtpLnk ml-3 mt-1">Xem tất cả đánh giá<span> › </span></a>
      </div> 